...
package com.android.calculator2.fragment;
...
import com.android.calculator2.adapter.HistoryAdapter;
import com.android.calculator2.expression.CalculatorExpr;
import com.android.calculator2.expression.Evaluator;
import com.android.calculator2.model.HistoryItem;
import com.android.calculator2.view.DragController;
...
public class HistoryFragment extends Fragment implements DragLayout.DragCallback {
    ...
    private Animation mRotateAnimUp, mRotateAnimDown;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mRotateAnimUp = AnimationUtils.loadAnimation(getActivity(), R.anim.rotate_up);
        mRotateAnimUp.setInterpolator(new LinearInterpolator());
        mRotateAnimDown = AnimationUtils.loadAnimation(getActivity(), R.anim.rotate_down);
        mRotateAnimDown.setInterpolator(new LinearInterpolator());
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mDragIcon = view.findViewById(R.id.drag_icon);
        mDragIcon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mDragLayout != null && mDragLayout.isOpen()) {
        ...
        toolbar.setOnMenuItemClickListener(new Toolbar.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                if (item.getItemId() == R.id.menu_clear_history) {
                    final CalculatorActivity calculator = (CalculatorActivity) getActivity();
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        final CalculatorActivity activity = (CalculatorActivity) getActivity();
    ...
    public void onInstanceStateRestored(boolean isOpen) {
        if (isOpen) {
            ...
            if (mIsArrowDown) {
                mIsArrowDown = false;
                mDragIcon.startAnimation(animateRotate(mIsArrowDown, false));
            } else if (!mIsArrowDown) {
                mIsArrowDown = true;
                mDragIcon.startAnimation(animateRotate(mIsArrowDown, true));
            }
        }
    }
    ...
    public void whileDragging(float yFraction) {
        if (yFraction > 0.5F && mIsArrowDown) {
            mIsArrowDown = false;
            mDragIcon.startAnimation(animateRotate(mIsArrowDown, false));
        } else if (yFraction <= 0.5F && !mIsArrowDown) {
            mIsArrowDown = true;
            mDragIcon.startAnimation(animateRotate(mIsArrowDown, true));
        }
    ...
    private Animation animateRotate(boolean isDown, boolean isClear) {
        Animation anim = isDown ? mRotateAnimDown : mRotateAnimUp;
        if (anim == null) {
            anim = AnimationUtils.loadAnimation(getActivity(), isDown ? R.anim.rotate_down : R.anim.rotate_up);
